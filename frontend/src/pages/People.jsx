...
const People = () => {
  ...
  const fetchPeople = async (nextPage = 1) => {
    setLoading(true)
    try {
      const res = await fetch(`https://api.themoviedb.org/3/person/popular?language=en-US&page=${nextPage}`, options)
      const json = await res.json()
      setPeople(prev => nextPage === 1 ? (json.results || []) : [...prev, ...(json.results || [])])
    } catch (e) {
      console.error(e)
    } finally {
      setLoading(false)
  ...
  return (
    <div className="min-h-screen bg-[#181818] text-white p-6 md:p-8">
      ...
      <div className="flex justify-center mt-8">
        <button
          disabled={loading}
          onClick={() => { const np = page + 1; setPage(np); fetchPeople(np) }}
